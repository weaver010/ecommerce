...
class PaypalController extends Controller
{
    ...
    public function paypal(){
        if(Session::has('order_id')){
            $orderDetails = Order::with('orders_products','user')->where('_id',Session::get('order_id'))->first()->toArray();
    ...
    public function pay(Request $request){
        try{
            // $orderDetails = Order::where('_id',Session::get('order_id'))->first()->toArray();
    ...
    public function success(Request $request){
        if ($request->input('paymentId') && $request->input('PayerID')) {
            ...
            if ($response->isSuccessful()) {
                ...
                Order::where('_id',$order_id)->update(['order_status'=>'Payment Captured']);
                ...
                Mail::send('emails.order',$messageData,function($message)use($email){
                    $message->to($email)->subject('Order Confirmed - SiteMakers.in');
                });
